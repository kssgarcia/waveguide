...
c--------------------------------------------------------------------
      ...
      Parameter (eps=0.00001)   ! used for point checks
      ...
      Do i=2,nelm          !  loop over elements
       Do j=1,6            ! loop over element nodes
        ...
        iflag=0
         ...
         Do k=1,npts
          If(Dabs(x(i,j)-p(k,1)).lt.eps) then
           If(Dabs(y(i,j)-p(k,2)).lt.eps) then
            If(Dabs(z(i,j)-p(k,3)).lt.eps) then
      ...
      Do i=1,npts                 !  loop over global nodes
       ...
       Do j=1,nelm        ! loop over elements
        Do k=1,6          ! loop over element nodes
         ...
         If(Dabs(p(i,1)-x(j,k)).lt.eps) then
          If(Dabs(p(i,2)-y(j,k)).lt.eps) then
           If(Dabs(p(i,3)-z(j,k)).lt.eps) then
            ...
            If(i.eq.23) then
             write (6,*) i,j,k,ne(i,Icount),Icount
            End If
