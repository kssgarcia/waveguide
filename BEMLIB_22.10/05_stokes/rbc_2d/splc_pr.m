...
%  x .... x coordinate of prescribed data
%  f .... function value of prescribed data
%
...
%-----
  ...
  for i=1:N-1
     rhs(i) = 3.0*( (f(i+2)-f(i+1))/h(i+1) - (f(i+1)-f(i))/h(i) );
  end
  rhs(N) = 3.0*( (f(2)-f(1))/h(1) - (f(N+1)-f(N))/h(N) );
...
for i=1:N
  ...
  c(i) = (f(i+1)-f(i))/h(i) - h(i)*(b(i+1)+2.0*b(i))/3.0;
end
