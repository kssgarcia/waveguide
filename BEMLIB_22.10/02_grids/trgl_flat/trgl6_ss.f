...
c containing a square hole with side "a"
c in the xy plane
c
c The patch is confined in: -0.5 < x,y <0.5
c
c Will subdivide a twelve-element pattern
...
c Iedge(i,1) = 2 for the inner square nodes
c--------------------------------
      ...
      Do i=1,npts
       ...
       if(abs(p(i,1)+side).lt.eps) Iedge(i,1) = 1
       if(abs(p(i,2)+side).lt.eps) Iedge(i,1) = 1
       if(abs(p(i,1)-a).lt.eps) Iedge(i,1) = 2
       if(abs(p(i,2)-a).lt.eps) Iedge(i,1) = 2
       if(abs(p(i,1)+a).lt.eps) Iedge(i,1) = 2
       if(abs(p(i,2)+a).lt.eps) Iedge(i,1) = 2
      End Do
