...
c--------------------------------------------
c Triangulation of the unit disk 
c in the xy plane into 6-node triangles
c by the subdivision of a four-element pattern
c
c SYMBOLS:
c -------
c
...
c-----
c project the outermost points to a circle
c-----

      Do k=1,nelm
        Do l=1,6
         if(efl(k,l).eq.1) then
           actis = sqrt(x(k,l)**2+y(k,l)**2)
           x(k,l) = x(k,l)/actis
           y(k,l) = y(k,l)/actis
          end if
        End Do
...
c----------------
      ...
      Do i=1,ndiv
       ...
       num = 0       ! count the new elements arising by sub-division
...
c---------------------------------
c number of elements has increased
...
c---
       ...
       Do k=1,nelm   ! reposition the mid-nodes
         if(efl(k,4).eq.0) then
           x(k,4)=0.5*(x(k,1)+x(k,2))
           y(k,4)=0.5*(y(k,1)+y(k,2))
         end if
         if(efl(k,5).eq.0) then
           x(k,5)=0.5*(x(k,2)+x(k,3))
           y(k,5)=0.5*(y(k,2)+y(k,3))
         end if
         if(efl(k,6).eq.0) then
