function [Npts,Nelm,p,ne,n,nbe] = trgl6_icos (Ndiv)
...
%                           
%  n(i,j) .... global node lable of local node number j
%               on element i,
%              where j=1,...,6
%
%  ne(i,j) ... ne(i,1) is the number of elements
%              touching global node i.
%              ne(i, 2:ne(i,1)+1) are the
...
%---
  ...
  ru = 0.25*sqrt(10.0+2.0*sqrt(5.0));
  rm = 0.25*(1.0+sqrt(5.0));   % twice the golden ratio
  c0 = 0.0;
  c1 = ru;
  c2 = 2.0*ru/sqrt(5.0);
  c3 = rm;
...
%------------------------------------------
% compute the mid-nodes  of the three sides
% of the 20 first-generation elements
%
% mid-nodes are numbered 4, 5, 6
%------------------------------------------
...
%---
        ...
        zn(nm,5) = 0.5*(zn(nm,2)+zn(nm,3));

        xn(nm,6) = 0.5*(xn(nm,3)+xn(nm,1));
        yn(nm,6) = 0.5*(yn(nm,3)+yn(nm,1));
        zn(nm,6) = 0.5*(zn(nm,3)+zn(nm,1));
        ...
        xn(nm,4) = 0.5*(xn(nm,1)+xn(nm,2));    % mid points
        yn(nm,4) = 0.5*(yn(nm,1)+yn(nm,2));
        zn(nm,4) = 0.5*(zn(nm,1)+zn(nm,2));
...
%---
      ...
      for i=1:Npts 
       ...
       for j=1:Nelm     % loop over elements
        for k=1:6       % loop over element nodes
