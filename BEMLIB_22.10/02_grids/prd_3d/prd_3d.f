...
c--------------------------------------------------
c  Adaptive representation of closed 3-D line.
c
...
c
c  Criteria for redistribution include:
c
...
c            replaced by a single point,
c            unless the resulting distribution violates either
...
c-----------------
      ...
      i3 = i+3
c---
c  examine the arc defined by points i-1, i, i+1
c---
...
c---
c compute the location of the point 
c located midway between i and i+1 
c---
...
c---
      ...
      Do j=NSG,i2,-1           !  renumber the points
        ja    = j-1
        x (j) = x (ja)
        y (j) = y (ja)
        z (j) = z (ja)
...
c-----------------------
      ...
      Do i=2,NSG
       ia = i-1
       s(i) = s(ia)+0.5D0*(R(i) *abs(chi1(i))
     +                    +R(ia)*abs(chi3(i)))
      End Do

      i = NSG1
      s(i) = s(NSG)+0.5D0*(R(i)*abs(chi1(i))
     +                  +R(1)*abs(chi3(1)))
...
c-----
 ...
 200  Format (' prd_3d: ARC',I3,' IS TOO BIG',2(F10.5))
 201  Format (' prd_3d: EXECUTION WILL BE INTERRUPTED')
