"""
Theorem 2.1 — discrete trapped mode discovered with Beyn's contour method.
...
Workflow
--------
1. Use Beyn on one contour enclosing the discrete spectral band.
2. Keep near-real eigenvalues inside 0 < kb < sqrt(Lambda_1)*b.
3. Locally refine each Beyn candidate by minimizing sigma_min(A(kb)) on the
   real axis.  This is verification, not global discovery.
4. Repeat the local verification for several BEM meshes M.
5. Count the persistent resolved modes and compare the branch nearest the
...
class Config:
    ...
    # Number of complex contour quadrature nodes. The trapezoidal rule is
    # spectrally accurate for analytic periodic integrands, so 80-128 is often
    # a useful practical range. Increase if contour convergence is poor.
    beyn_quadrature_points: int = 96
    ...
    # Relative singular-value threshold used to estimate rank(S0).
    ...
    beyn_cutoff_margin: float = 1.0e-7
    ...
    beyn_real_axis_tolerance: float = 2.0e-4
    ...
    output_directory: str = "theorem_2_1_beyn_validation"
...
def complex_support_smoke_test(epsilon: float, config: Config) -> None:
    """
    Verify that the project Green/lattice implementation can assemble A(z) for
    a genuinely complex z. Beyn cannot be used otherwise.
    """
    ...
    except Exception as exc:  # noqa: BLE001 - explicit diagnostic for project code
        raise RuntimeError(
            "Beyn requires A(kb) for complex kb, but the current lattice_sums / "
            "Green-function implementation failed the complex-kb smoke test at "
            f"kb={z_test!r}. Original error: {type(exc).__name__}: {exc}"
        ) from exc
    ...
    if not np.all(np.isfinite(A.real)) or not np.all(np.isfinite(A.imag)):
        raise RuntimeError(
            ...
            "The Green-function analytic continuation must be fixed before Beyn."
        )


def probing_matrix(M: int, config: Config) -> np.ndarray:
    if config.beyn_probe_dim >= M:
        raise ValueError("beyn_probe_dim must be strictly smaller than the BEM size M.")
    ...
    # Orthonormal columns improve scaling of the contour moments.
    Q, _ = np.linalg.qr(V)
...
def beyn_discover(
    epsilon: float,
    config: Config,
) -> tuple[np.ndarray, np.ndarray, BeynDiagnostics, list[BeynEigenvalueRow]]:
    """
    Run one Beyn contour solve.

    Returns
    -------
    raw_eigenvalues:
        Eigenvalues of the reduced Beyn matrix.
    s0_singular_values:
        Singular values used to estimate the enclosed spectral rank.
    diagnostics:
        Numerical diagnostics for the contour solve.
    rows:
        Classification of every raw eigenvalue.
    """
    M = config.beyn_M
    Nq = config.beyn_quadrature_points
    ...
    # Midpoint-shifted periodic nodes avoid evaluating exactly at the real-axis
    # ellipse endpoints, which are intentionally close to spectral thresholds.
    for j in range(Nq):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Move/enlarge the contour so no eigenvalue lies on Gamma, or "
        ...
        # (1/(2*pi*i)) integral ... dz, with dtheta=2*pi/Nq:
        # weight = (dz/dtheta)/(i*Nq).
        weight = dz_dtheta / (1j * Nq)
        S0 += weight * X
        S1 += weight * z * X

        if (j + 1) % 16 == 0 or j + 1 == Nq:
    ...
    for idx, eig in enumerate(raw_eigenvalues):
        inside = point_inside_ellipse(complex(eig), config)
        inside_real_band = bool(left < eig.real < right)
        near_real = bool(abs(eig.imag) <= config.beyn_real_axis_tolerance)
        accepted = bool(inside and inside_real_band and near_real)
        near_real_count += int(accepted)
        rows.append(
            BeynEigenvalueRow(
                epsilon=epsilon,
                raw_index=idx,
    ...
    diagnostics = BeynDiagnostics(
        ...
        near_real_candidates=near_real_count,
    )
    ...
    if len(raw_eigenvalues):
        ...
        for idx, eig in enumerate(raw_eigenvalues):
            tag = "real-candidate" if rows[idx].accepted_for_real_refinement else "other"
            print(f"    [{idx}] {eig.real:+.12f} {eig.imag:+.3e}i  {tag}")
    else:
        print("  raw Beyn eigenvalues: none")

    return raw_eigenvalues, s, diagnostics, rows
...
def cluster_real_candidates(
    ...
    # Average each tiny numerical cluster.
...
def validate_epsilon(
    ...
    raw_eigs, s0_singular_values, beyn_diag, eigen_rows = beyn_discover(epsilon, config)
    candidates = cluster_real_candidates(raw_eigs, eigen_rows, config)
    brackets = candidate_brackets(candidates, config)

    print(f"  clustered near-real Beyn candidates = {len(candidates)}")
    ...
    for i, (candidate, bracket) in enumerate(zip(candidates, brackets, strict=True), start=1):
        rows, mode = run_candidate_refinement(
            epsilon, i, candidate, bracket, config
        )
        all_refinement.extend(rows)
        mode_results.append(mode)

    resolved = [mode for mode in mode_results if mode.resolved]
    unique_mode = len(resolved) == 1

    if resolved:
        # The theorem branch should be the resolved mode nearest the asymptotic
        # prediction. This remains only a labeling step; Beyn discovered it
        # independently of the asymptotic formula.
    ...
    result = ValidationResult(
        ...
        beyn_estimated_rank=beyn_diag.estimated_rank,
        ...
        unique_mode_verified=unique_mode,
...
def plot_beyn_contour(
    ...
    if eigen_rows:
        ...
        plt.scatter(raw.real, raw.imag, marker="x", label="raw Beyn eigenvalues")
...
def plot_s0_singular_values(
    ...
    plt.title(rf"Beyn rank diagnostic, $\varepsilon={epsilon:.2f}$")
    plt.tight_layout()
...
def plot_summary(results: list[ValidationResult], output_directory: Path, config: Config) -> None:
    ...
    plt.ylabel("resolved discrete modes")
    plt.title("Beyn + BEM trapped-mode count")
    plt.tight_layout()
...
def print_result(result: ValidationResult, config: Config) -> None:
    ...
    print(f"  Beyn estimated contour rank    = {result.beyn_estimated_rank}")
    print(f"  near-real Beyn candidates      = {result.beyn_near_real_candidates}")
    print(f"  resolved discrete modes        = {result.resolved_mode_count}")
    print(
        "  exactly one resolved mode      = "
...
def main() -> None:
    ...
    print("=== Theorem 2.1 validation with Beyn contour discovery ===")
    print(f"b = {config.b}")
    print(f"a = {config.a}")
    ...
    print(
        f"Beyn settings: M={config.beyn_M}, Nq={config.beyn_quadrature_points}, "
        f"probe_dim={config.beyn_probe_dim}"
    )
    ...
    # One smoke test is enough because complex support is a property of the
    # Green/lattice implementation, not of epsilon itself.
    ...
    for epsilon in config.epsilon_values:
        ...
        all_beyn_diagnostics.append(beyn_diag)
        ...
        plot_beyn_contour(epsilon, eigen_rows, output_directory, config)
        plot_s0_singular_values(epsilon, s0_singular_values, output_directory, config)
    ...
    uniqueness_passed = sum(row.unique_mode_verified for row in summaries)
    asymptotic_passed = sum(row.asymptotic_agreement_verified for row in summaries)

    print("\n=== FINAL SUMMARY ===")
    print("Global discovery: Beyn contour method")
    print("Local certification: SVD minima + BEM mesh refinement")
    print("Interval enclosed: 0 < k^2 < Lambda_1, excluding tiny endpoint margins")
    print(
        f"Exactly one resolved discrete mode: "
        f"{uniqueness_passed}/{len(summaries)}"
    )
    print(
    ...
    for row in summaries:
        print(
            ...
            f"Beyn-rank={row.beyn_estimated_rank}, "
