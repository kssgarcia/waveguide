"""
Focused numerical validation of Theorem 2.3(iii): one x-symmetric BIC.

The theorem states that, for sufficiently small epsilon, if the obstacle
Gamma is symmetric with respect to the x-axis (a = 0, X even, Y odd), then
nu = 0 automatically and there exists a unique eigenvalue in
...
This script is intentionally restricted to statement (iii). It mirrors the
structure used for the focused validation of Theorem 2.1, but adds the key
extra diagnostic required for an embedded eigenvalue/BIC: the coefficient of
the first propagating transverse waveguide mode must vanish numerically.
...
3. BIC / NON-RADIATION TEST
   Is the reconstructed field orthogonal, to numerical tolerance, to the
   first propagating transverse Dirichlet mode on far cross-sections?
   For this x-symmetric case the BIC is also expected to have odd parity in y.
...
Important:
- sv_min(A) below denotes the smallest singular value of the BEM matrix.
- sigma denotes the spectral distance in Theorem 2.3. They are different.
- The diagnostic tolerances below are numerical reporting criteria, not
  constants appearing in the theorem.
...
class Config:
    # Guide geometry.
    b: float = 1.0
    a: float = 0.0  # Required by Theorem 2.3(iii).

    # Shape used in the previous Theorem 2.3 implementation:
    #   X0(t) = cos(t) - beta/2 cos(2t)  (even)
    #   Y0(t) = sin(t) - beta/2 sin(2t)  (odd)
    # The physical obstacle is epsilon*(X0,Y0).
    # beta != 0 avoids accidentally imposing y-axis symmetry as well.
    ...
    # BIC diagnostics.
    # These are numerical tolerances; inspect the reported values before
    # tightening them for a paper.
    odd_parity_tolerance: float = 1.0e-2
    ...
    output_directory: str = "theorem_2_3_x_symmetry_validation"
...
class RefinementRow:
    ...
    odd_parity_residual: float
    even_parity_residual: float
...
def dipole_moments(shape_beta: float) -> tuple[float, float]:
    ...
    This follows the same project convention as the previous Theorem 2.3
    script. The epsilon scaling is handled separately by the asymptotic law.
    """
    mu, nu = dip.dipole(shape_beta, 0.0, 0.0, "x")
    return float(mu), float(nu)


def asymptotic_prediction(epsilon: float, config: Config) -> tuple[float, float]:
    """Return (kb_asymptotic, sigma_asymptotic) for Theorem 2.3(iii)."""
...
def kb_from_delta_2(delta_2: float, config: Config) -> float:
    """
    delta_2 = Lambda_2-k^2 = sigma^2.
    Return kb while remaining inside Lambda_1 < k^2 < Lambda_2.
    """
...
def obstacle_geometry(
    ...
    """
    x-axis-symmetric obstacle:

      X(t) = eps [cos t - beta/2 cos(2t)]   (even)
      Y(t) = eps [sin t - beta/2 sin(2t)]   (odd)

    The center is at a=0, as required by Theorem 2.3(iii).
    """
    t = np.asarray(t)
    beta = config.shape_beta

    X = epsilon * (np.cos(t) - 0.5 * beta * np.cos(2.0 * t))
    Y = epsilon * (np.sin(t) - 0.5 * beta * np.sin(2.0 * t))

    Xp = epsilon * (-np.sin(t) + beta * np.sin(2.0 * t))
    Yp = epsilon * (np.cos(t) - beta * np.cos(2.0 * t))

    Xpp = epsilon * (-np.cos(t) + 2.0 * beta * np.cos(2.0 * t))
    Ypp = epsilon * (-np.sin(t) + 2.0 * beta * np.sin(2.0 * t))
...
def geometry_symmetry_residual(config: Config) -> tuple[float, float]:
    ...
    x_even = float(np.max(np.abs(Xm - Xp)))
    y_odd = float(np.max(np.abs(Ym + Yp)))
    return x_even, y_odd
...
def boundary_parity_residuals(boundary_vector: np.ndarray) -> tuple[float, float]:
    """
    Reflection across the x-axis is theta -> 2pi-theta.
    With midpoint nodes this maps j -> M-1-j.

    The BIC associated with the second transverse threshold is expected to be
    odd in y, hence u(theta_reflected) = -u(theta).
    """
    ...
    return odd_residual, even_residual
...
def first_propagating_component(
    ...
    For Dirichlet walls y=+-b and a=0, the first transverse mode is

        phi_1(y) = cos(pi y / (2b)).

    A true x-symmetric BIC must have zero coefficient in this propagating
    channel. Numerically we report the projected norm divided by the total
    cross-sectional field norm on x=+-x_far.
    """
    b = config.b
    ...
    y_values = b * nodes
    physical_weights = b * weights

    phi_1 = np.cos(PI * y_values / (2.0 * b))
...
def refine_expected_mode_for_M(
    ...
    Return
      ...
      odd_parity_residual, even_parity_residual, propagation_diagnostics.
    """
...
def run_expected_branch_refinement(
    ...
    for M in config.refinement_M:
        ...
        print(
            ...
            f"odd_res={odd_residual:.3e}, Rprop={max_prop:.3e}, "
...
def expected_bic_is_resolved(
    ...
    The first tests the BEM eigenvalue branch. The second additionally checks
    the symmetry/non-radiation diagnostics that distinguish the BIC in the
    open spectral band.
    """
    if len(rows) < 2:
        return False, False
    ...
    nonradiating_bic = bool(spectral_resolved and odd_ok and propagation_ok)
...
def whole_band_uniqueness_screen(
    ...
    for j, (left, right) in enumerate(other_brackets, start=1):
        ...
        if np.isfinite(kb_f) and len(vector_f) > 0:
            odd_residual, _ = boundary_parity_residuals(vector_f)
            propagation = first_propagating_component(
        ...
        persistent_nonradiating = bool(
            ...
            and np.isfinite(odd_residual)
            and odd_residual <= config.odd_parity_tolerance
        ...
        print(
            ...
            f"shift={kb_shift:.3e}, odd_res={odd_residual:.3e}, "
            f"Rprop={max_prop:.3e}, "
...
def symmetry_conditions_verified(
    ...
    passed = bool(
        abs(config.a) <= config.geometry_symmetry_tolerance
        and x_even_residual <= config.geometry_symmetry_tolerance
        and y_odd_residual <= config.geometry_symmetry_tolerance
        and abs(nu) <= config.nu_tolerance
    )
    return passed, x_even_residual, y_odd_residual, mu, nu
...
def validate_epsilon(
    ...
    result = ValidationResult(
        ...
        odd_parity_residual=final.odd_parity_residual,
...
def plot_full_band_scan(
    ...
    plt.title(rf"Theorem 2.3(iii), $\varepsilon={epsilon:.2f}$")
    plt.legend()
    plt.tight_layout()
...
def plot_refinement(
    ...
    odd = np.array([row.odd_parity_residual for row in rows])
    prop = np.array(
    ...
    plt.figure(figsize=(7, 4.5))
    plt.semilogy(M, odd, "o-")
    plt.axhline(
        config.odd_parity_tolerance,
        linestyle="--",
        label="odd-parity tolerance",
    )
    plt.xlabel(r"$M$")
    plt.ylabel(r"odd-parity residual")
...
def print_result(result: ValidationResult, config: Config) -> None:
    ...
    print("\n  --- validation result ---")
    print(
        f"  symmetry conditions (a=0, X even, Y odd, nu~0): "
    ...
    print(f"  odd-parity residual = {result.odd_parity_residual:.3e}")
...
def main() -> None:
    ...
    symmetry_ok, x_even_res, y_odd_res, mu, nu = symmetry_conditions_verified(config)

    print("=== Focused validation of Theorem 2.3(iii): x-symmetric BIC ===")
    print(f"b = {config.b}")
    print(f"a = {config.a}")
    ...
    if not symmetry_ok:
        raise RuntimeError(
            "The selected obstacle does not satisfy the numerical symmetry / nu=0 checks."
        )
    ...
    for row in summary:
        ...
        print(
            ...
            f"odd_res={row.odd_parity_residual:.3e}, "
            f"Rprop={max_prop:.3e}, "
