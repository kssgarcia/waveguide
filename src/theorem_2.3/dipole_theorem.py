...
def gl_quad_greens(point_a, point_b, point_0):
    ...
    G, Gx, Gy = greens_parametrized(gauss_legendre.z, point_a, point_b, point_0)
...
def potential(point_0, bound_param):
    ...
    sum = 0
    for i in range(Ne):
        alpha, beta = influence_coefficients(
            P_long[:, i], P_long[:, i + 1], point_0, is_singular=False
        )
...
def dipole(beta, xc, yc, symm="x"):
    Ne = 100
    t = np.linspace(0, 2 * pi, num=Ne + 1)

    if symm == "x":
        X_long = (np.cos(t) - (beta / 2) * np.cos(2 * t)) + xc
        Y_long = (np.sin(t) - (beta / 2) * np.sin(2 * t)) + yc
    else:
        X_long = (np.sin(t) - (beta / 2) * np.sin(2 * t)) + xc
        Y_long = (-np.cos(t) + (beta / 2) * np.cos(2 * t)) + yc
    ...
    for i in range(Ne):
        ...
        for j in range(Ne):
            point_a = P_long[:, j]
            point_b = P_long[:, j + 1]
            A[i, j], B[i, j] = influence_coefficients(point_a, point_b, point_0, i == j)
