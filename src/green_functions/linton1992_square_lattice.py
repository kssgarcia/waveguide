...
def rho(t):
    ...
    a square of side 2*a.

    The expected range of t is -pi/4 < t < 7*pi/4
    """
    if -pi/4 <= t < pi/4:
        return a/cos(t)
    elif t < 3*pi/4:
        ...
        print('t out of expected range')
        sys.exit(1)

# rho_vec = np.vectorize(rho)
# tt = np.linspace(-pi/4, 7*pi/4, 501, endpoint=False)
# rho_vals = rho_vec(tt)
# xx = rho_vals*np.cos(tt)
# yy = rho_vals*np.sin(tt)
# plt.figure(1)
# plt.clf()
# plt.plot(xx, yy, '.-')
...
theta = (np.arange(1,M+1)-0.5)*2*np.pi/M - pi/4

kb_list = np.array([1.4, 1.45, 1.47, 1.5])
...
for el, kb in enumerate(kb_list):
    ...
    G = lambda x, y, xi, eta: lattice.greens_neumann(x, y + b, xi, eta + b, S_2d, k, d)
    G_reg = lambda x, y, xi, eta: lattice.greens_neumann_reg(x, y + b, xi, eta + b, S_2d, k, d)
...
for det, kb in zip(det_list, kb_list):
    plt.text(np.real(det), np.imag(det), f'$kb={kb}$')
