...
def solve_helmholtz_eigenproblem(num_eigs=6, bc_type='mixed', N_modes=5, b=1.0, max_iter=10, tol=1e-6):
    ...
    for it in range(max_iter):
        ...
        # Solve
        eigvals, eigvecs_reduced = eigsh(A_bc, k=num_eigs, M=M_bc, sigma=k_guess**2, which='LM')
        ...
        # Update k_guess
        k_new = np.sqrt(np.abs(eigvals[0]))

        if abs(k_new - k_guess) < tol:
            print(f"Converged after {it+1} iterations, k = {k_new}")
            break
        k_guess = k_new
...
def plot_all_eigenmodes(nodes, elements, eigvecs, eigvals, bc_type='dirichlet_all'):
    ...
    for i in range(num_modes):
        ...
        # Plot the eigenmode
        cs = axes[i].tricontourf(triang, u, levels=50, cmap='jet', vmin=vmin, vmax=vmax)
...
if __name__ == "__main__":
    ...
    eigvals, eigvecs, nodes, elements, boundary_nodes = \
        solve_helmholtz_eigenproblem(num_eigs=num_eigs, bc_type=bc_type, N_modes=N_modes, b=b)

    top_nodes, bottom_nodes, left_nodes, right_nodes, obstacle_nodes = boundary_nodes

    mn_list = [(m, 1) for m in range(1, num_eigs + 1)]
