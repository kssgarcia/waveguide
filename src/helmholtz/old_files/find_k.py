...
import src.helmholtz.DtN_newton_correction as dtn
...
for k in kb_list:
    n_modes = int(np.floor(b * k / np.pi))
    u_full, interior_nodes, nodes, elements, boundary_nodes, A_bc, M_bc = \
        dtn.solve_helmholtz_eigenproblem(nodes, elements, bc_type=bc_type, N_modes=n_modes, b=b, k_guess=k)
