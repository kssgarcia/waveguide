...
def apply_boundary_conditions(A, M, nodes, boundary_nodes, bc_type='mixed'):
    ...
    if bc_type == 'dirichlet_all':
        ...
        remove_boundary_nodes = np.concatenate([left_nodes, right_nodes])
        dirichlet_nodes = np.unique(remove_boundary_nodes)
        ...
        dirichlet_nodes = np.concatenate([top_nodes, bottom_nodes, left_nodes])
...
def assemble_full_matrix(nodes, elements, bc_type, k2):
    N = len(nodes)
    ...
    A_bc, M_bc, interior_nodes = apply_boundary_conditions(A, M, nodes, boundary_nodes, bc_type)

    return interior_nodes, nodes_array, elements_array, boundary_nodes, A_bc, M_bc
...
if __name__ == "__main__":
    ...
    # r=0.2 # radio
    # lc=0.1 # Controla la finura del mallado
    # nodes, elements = gen.mesh_with_obstacle_center(L=L, b=b, xc=xc, yc=yc, r=r, lc=lc)
    
    bc_type = 'dirichlet_neumann'
    k2_list = np.array([1.8, 1.9, 2, 2.1, 2.3])
    s_list = np.zeros(len(k2_list))
    interior_nodes, nodes, elements, boundary_nodes, A_bc, M_bc = \
        assemble_full_matrix(nodes, elements, bc_type, k2=0)


    # Solve eigenvalue problem
    num_eigs=10
    eigvals, eigvecs_reduced = eigsh(A_bc, k=num_eigs, M=M_bc, which='SM')
    ...
    for i in range(num_eigs):
        print("k", eigvals[i]**0.5)
        u_full = eigvecs[:,i]
        plot_eigenmode(nodes, elements, u_full, title=f"i={i}")
        plt.show()
