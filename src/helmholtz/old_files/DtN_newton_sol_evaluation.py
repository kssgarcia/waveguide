"""
This code evaluate the solution of the code DtN_newton.py in the DtN.py file, that use
...
def load_mesh_meshio():
    ...
    mesh = meshio.read(f"{folder}/mesh_with_hole.msh")
...
def solve_helmholtz_eigenproblem(num_eigs=6, bc_type='mixed', N_modes=5, b=1.0, max_iter=10, tol=1e-6):
    ...
    A_mod = A.copy()
    # Assemble DtN with current k_guess
    B_right = assemble_DtN(nodes_array, right_nodes, k_guess, N_modes, b, sign=1)
    A_mod[right_nodes[:, None], right_nodes] -= B_right
    B_left = assemble_DtN(nodes_array, left_nodes, k_guess, N_modes, b, sign=-1)
    A_mod[left_nodes[:, None], left_nodes] -= B_left

    # Apply BCs
    A_bc, M_bc, interior_nodes = apply_boundary_conditions(A_mod, M, nodes_array, boundary_nodes, bc_type)

    # Solve
    eigvals, eigvecs_reduced = eigs(A_bc, k=1, M=M_bc, sigma=(k_guess**2 if k_guess!=0 else 0.0))
...
def plot_eigenmode(nodes, elements, u_full, title=None):
    ...
    cs = ax.tricontourf(triang, u_real, levels=50, cmap='viridis')
...
if __name__ == "__main__":
    ...
    eigvals, eigvecs, nodes, elements, boundary_nodes = \
        solve_helmholtz_eigenproblem(num_eigs=num_eigs, bc_type=bc_type, N_modes=N_modes, b=b, max_iter=1)

    top_nodes, bottom_nodes, left_nodes, right_nodes, obstacle_nodes = boundary_nodes

    # Visualize 
    plot_boundary_nodes(nodes, elements, boundary_nodes)
    plot_eigenmode(nodes, elements, eigvecs[:,0], title=f"Mode Re(u), k^2 = 0,99")
