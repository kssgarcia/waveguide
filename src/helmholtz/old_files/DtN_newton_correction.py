...
def solve_system(A, M, nodes, elements, boundary_nodes,
    ...
    try:
        eigvals, eigvecs = eigs(A_bc, k=1, M=M_bc, sigma=k**2)
    except Exception:
    ...
    return u_full, interior_nodes, A_bc, M_bc

def solve_helmholtz_eigenproblem(nodes, elements, bc_type='mixed', N_modes=5, b=1.0, k_guess=1.0, max_iter=10, tol=1e-6):
    N = len(nodes)
    ...
    u_full, interior_nodes, A_bc, M_bc = solve_system(
    ...
    return u_full, interior_nodes, nodes_array, elements_array, boundary_nodes, A_bc, M_bc
...
if __name__ == "__main__":
    ...
    nodes, elements = gen.mesh_with_obstacle_center(L=L, b=b, a=a, xc=xc, yc=yc, r=r, lc=lc)

    u_full, interior_nodes, nodes, elements, boundary_nodes, A_bc, M_bc = \
        solve_helmholtz_eigenproblem(nodes, elements, bc_type=bc_type, N_modes=N_modes, b=b, k_guess=k_guess)

    # LHS = (A_bc - k_guess**2 * M_bc)
    # _, s, _ = svds(LHS, k=1, which='SM')

    # print(f"\n{'='*50}")
    # print(f"Smallest singular value: {s[0]}")
    # print(f"\n{'='*50}")

    print(f"Computed eigenmode for k = {k_guess}")
    plot_boundary_nodes(nodes, elements, boundary_nodes)
    plot_eigenmode(nodes, elements, u_full, title=f"Mode Re(u), k = {k_guess}")
