...
df = pd.DataFrame({
    ...
    "kb_analytic": kb_analytic_list,
    "kb_root": kb_list,
    "error": error_list,
})
df.to_csv("kevin_kb.csv", index=False)
...
color1 = 'tab:blue'
ax1.plot(epsilon_list, kb_list, 'o-', color=color1, label=r'$kb_{sol}$', markersize=6)
ax1.axhline(0, color='r', linewidth=1)
ax1.set_xlabel(r"$\epsilon$", fontsize=12)
ax1.set_ylabel(r"$kb_{sol}$", color=color1, fontsize=12)
